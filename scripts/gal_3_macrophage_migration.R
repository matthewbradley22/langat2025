...
############# Split by celltype (plots for figure) #############
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
...
#Plot perithelial markers in pericytes
peri_trafficking_dot_dat <- DotPlot(wt_peri, features = c('Vcam1', 'Icam1', 'Icam2', 'Sele', 'Selp', 'Mcam', 'F11r', 'Jam2', 'Pecam1', 'Pvr', 'Cd99l2',
                                                                 'Cdh5', 'Cxcl12'), group.by = 'treatment_time', scale = FALSE)$data
meta_columns <- str_split_fixed(peri_trafficking_dot_dat$id, " ", 2)
colnames(meta_columns) <- c('treatment', 'time')
peri_trafficking_dot_dat <- cbind(peri_trafficking_dot_dat, meta_columns)

pdf('~/Documents/ÖverbyLab/scPlots/galectin3_proj/peri_trafficking_dot.pdf', width = 8, height = 6)
ggplot(peri_trafficking_dot_dat, aes(x = time, y = features.plot))+
  geom_point(aes(size = pct.exp, fill = avg.exp.scaled), pch = 21)+
  facet_grid(cols = vars(treatment), scales = "free")+
  #coord_flip()+
  scale_size_continuous(range = c(0.5,6), limits = c(0,100))+
  scale_fill_gradientn(colours = c("#F03C0C","#F57456","#FFB975","white"), 
                       values = c(1.0,0.7,0.4,0),
                       limits = c(0,2.5))+
  ggtitle('pericyte genes (only 148 cells total)')+
  theme_bw()+
  theme(panel.grid = element_blank(),
        #panel.border = element_rect(colour = "white", fill = NA),
        panel.spacing = unit(0, "line"),
